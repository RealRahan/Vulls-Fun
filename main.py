...
{prefix}text `تكتب شي بملف تيكست ك اختصار سريع`
...
async def text(ctx,*, arg=None):
 if arg == None:
  await ctx.send(f"**{prefix}text انا بملف تيكست!")
 with open('text.txt', 'w') as f:
  f.write(arg)
 await ctx.reply(file=discord.File("text.txt"), mention_author=False)
