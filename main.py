...
{prefix}say `التكلم بالبوت بروم مخصص`
**"""),
 discord.Embed(title=f"الأوامر الأشرافية", description="""**
...
async def lyrics(ctx,*, arg):
 ...
 if len(r.json()["lyrics"]) > 4096:
 	await ctx.send(f"**الأغنية تحتوي على {len(r.json()['lyrics'])} حرف من أصل 4096 وهذا هو العدد المسموح للديسكورد.**")
 	return
 confirmation = BotConfirmation(ctx)
 ask=await confirmation.confirm(f"**هل انت متأكد من ذلك؟ ، الأغنية تحتوي على {len(r.json()['lyrics'])} حرف و قد يحدث ازعاج في روم {ctx.channel.name}**")
 if confirmation.confirmed:
  await confirmation.update("**جارِ تحميل الكلمات ...**", hide_author=True)
