...
async def userstatus(ctx,*, member: discord.Member=None)
 ...
 if r.json()["status"] == 200:
  r=requests.get(f"https://normal-api.tk/userstatus?userid={member.id}")
  status=discord.Embed(title=r.json()["username"], description=f"""
الأسم مع التاق: {r.json()['discrim']
حالة الاتصال: {r.json()['user_status']
الحالة: {r.json()['status_type']
""", color=ctx.author.color)
 else:
  await ctx.send("**مالقيت هذا العضو**")
