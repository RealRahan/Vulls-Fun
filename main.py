...
async def nazi(ctx, member: discord.Member=None):
 if member==None:
  member=ctx.author
 chad=discord.Embed(title="**Nazi Rate**", description=f"**{member.name} نازي بنسبة {random.randint(-10, 100)}%**", color=ctx.author.color)
 chad.set_thumbnail(url="https://thumbs.gfycat.com/ThirstyMadeupIchthyostega-size_restricted.gif")
 await ctx.reply(embed=chad, mention_author=False)
