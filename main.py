...
async def system(ctx):
 ...
 info=discord.Embed(title="**معلومات النظام**", description=f"""**
النظام: {platform.platform()}
المعالج: {platform.processor()}
استخدام المعالج: {psutil.cpu_percent(4)}%
إجمالي الرام: {total_memory} مب
الرام المستخدم: {used_memory} مب
الرام المتاح: {free_memory} مب
المساحة التخزينية الإجمالية: {total // (2**30)} ج.ب
المساحة التخزينية المستخدمة: {used // (2**30)}
المساحة التخزينية المتاحة: {free // (2**30)}
**""", color=ctx.author.color)
 m=await ctx.reply(embed=info, mention_author=False)
 while True:
  time.sleep(1)
  info=discord.Embed(title="**معلومات النظام**", description=f"""**
النظام: {platform.platform()}
...
الرام المتاح: {free_memory} مب
المساحة التخزينية الإجمالية: {total // (2**30)} ج.ب
المساحة التخزينية المستخدمة: {used // (2**30)} ج.ب
المساحة التخزينية المتاحة: {free // (2**30)} ج.ب
**""", color=ctx.author.color)
 await m.edit(embed=info)
